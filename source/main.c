...
static uint32_t BIG_TO_HOST(uint32_t x) { return (((x & 0xFF) << 24) | ((x & 0xFF00) << 8) | ((x & 0xFF0000) >> 8) | ((x & 0xFF000000) >> 24)); }

#define LOWORD(x) (*((uint16_t*)&(x)))

static uint16_t __ROL2__(uint16_t value, int count) {
    if (count >= 0) {
...
int main(int argc, char **argv)
{
    ...
    uint8_t *outbuf; // eax
    uint8_t *outbuf1; // esi
    unsigned int input_content_big; // eax
    ...
    char destination; // [esp+90h] [ebp-1000h]
    ...
    input_content_big = BIG_TO_HOST(input_content1[2]);
    v68 = 0;
    v71 = 0;
    v67 = 16;
...
LABEL_69:
    ...
    v52 = fopen(&destination, "wb");
    v53 = v52;
    if (v52)
    {
        v54 = fwrite(outbuf1, 1u, Countb, v52);
        printf("Wrote:%d\n", v54);
        fclose(v53);
        result = 1;
    }
